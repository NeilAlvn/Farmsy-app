...
- **DISPUTED (this session):** the arrow is a **standard Material `ArrowDownward`** per Neil's brief; Aviah's thread specifies a hand-drawn arrow. iOS build 23 hand-drew a Path beside the button (wrong shape + placement); Android corrects to a regular arrow above the button. See PORT_NOTES. Also: iOS build 23 added a "Thank you, we have your answers." just-answered celebration not in the thread; Android follows the thread (single FEEDBACK screen). iOS to reconcile.
